...
public class EngageFullListener : EngageBaseListener
{
    ...
    private NC.Trigger ToTrigger(EngageParser.TriggerContext trigger)
    {
        ...
        string flag = trigger.Flag == null ? "" : trigger.Flag.Text;
        if (trigger.T != null)
            result = new NC.TerminalTrigger(trigger.T.Text, flag);
        else if (trigger.Bof != null)
            result = new NC.SpecialTrigger("BOF", flag);
        else if (trigger.Eof != null)
            result = new NC.SpecialTrigger("EOF", flag);
        else if (trigger.NT != null)
            result = new NC.NonterminalTrigger(trigger.NT.Text, flag);
        return result;
    }
    ...
    private NC.Reaction ToReaction(EngageParser.ReactionContext reaction)
    {
        switch (reaction.Command.Text)
        {
            ...
            case "trim":
                return new NC.TrimReaction
                    { Name = reaction.name().ID().GetText(), Starred = reaction.Star != null };
            case "pass":
